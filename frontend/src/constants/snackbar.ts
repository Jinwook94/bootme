const SNACKBAR_MESSAGE = {
  SUCCESS_LOGIN: '로그인 되었습니다.',
  DUPLICATION_CHECK_NEEDED: '닉네임 중복검사를 해주세요.',
  SUCCESS_PROFILE_UPDATE: '프로필이 수정되었습니다.',
  FAIL_PROFILE_UPDATE: '프로필 수정에 실패했습니다.',
  SUCCESS_PROFILE_IMAGE_UPDATE: '프로필 이미지가 수정되었습니다.',
  FAIL_PROFILE_IMAGE_UPDATE: '프로필 이미지 수정에 실패했습니다.',
  SUCCESS_ADD_BOOKMARK_COURSE: '북마크 저장되었습니다.',
  SUCCESS_DELETE_BOOKMARK_COURSE: '북마크 삭제되었습니다.',
  SUCCESS_COPY_URL_TO_CLIPBOARD: 'URL이 복사되었습니다.',
  FAIL_LOGIN: `로그인 실패`,
  NEED_LOGIN: `로그인 후 이용할 수 있습니다.`,
  WORK_IN_PROGRESS: `준비중입니다.`,
  SUCCESS_UPLOAD: `업로드 성공`,
  FAIL_UPLOAD: `업로드 실패`,
  FAIL_UPLOAD_IMAGE: `이미지 업로드 실패`,
  FAIL_COURSE_FETCH: `코스 조회에 실패했습니다`,
  FAIL_POST_UPLOAD: `게시글 등록에 실패했습니다`,
  FAIL_POST_FETCH: `게시글 조회에 실패했습니다`,
  FAIL_POST_EDIT: `게시글 수정에 실패했습니다`,
  FAIL_POST_DELETE: `게시글 삭제에 실패했습니다`,
  FAIL_COMMENT_DELETE: `댓글 삭제에 실패했습니다`,
  FAIL_COMMENT_EDIT: `댓글 수정에 실패했습니다`,
  SUCCESS_POST_DELETE: `게시글이 삭제되었습니다.`,
  SUCCESS_COMMENT_DELETE: `댓글이 삭제되었습니다.`,
  FAIL_COMMENTS_FETCH: `해당 게시글의 댓글 조회에 실패했습니다`,
  FAIL_VOTE: `투표에 실패했습니다`,
  SUCCESS_WRITE_COMMENT: `댓글 작성 완료`,
};

export default SNACKBAR_MESSAGE;

export const CHECK = 'check';
export const EXCLAMATION = 'exclamation';
